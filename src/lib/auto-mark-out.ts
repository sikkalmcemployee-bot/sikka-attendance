...
export async function processAutoMarkOut(targetEmployeeId?: string) {
  try {
    ...
    if (!db) {
      return { success: false, message: "Database unavailable", processedCount: 0 };
    }

    const attendanceCol = db.collection('attendance');
    const plantExitsCol = db.collection('plantExits');

    // Find active Open attendance records (optionally scoped to single employee)
    ...
    const openRecords = await attendanceCol.find(filter).toArray();
